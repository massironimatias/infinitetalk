...
This project is a template designed to easily deploy and use [InfiniteTalk](https://github.com/massironimatias/InfiniteTalk) in the RunPod Serverless environment.

[![Runpod](https://api.runpod.io/badge/massironimatias/infinitetalk_serverless)](https://console.runpod.io/hub/massironimatias/infinitetalk_serverless)
...
| `width`            | `integer` | No       | `512`                          | Width of the output video in pixels (e.g. `288`, `384`, `512`)                                                                                                                   |
| `height`           | `integer` | No       | `512`                          | Height of the output video in pixels (e.g. `512`)                                                                                                                               |
| `fps`              | `integer` | No       | `25`                           | Frame rate for the output video and audio sync alignment (e.g. `15`, `20`, `25`)                                                                                                |
...
| `s3_secret_key`    | `string`  | No       | Env: `S3_SECRET_KEY`           | S3 / Cloudflare R2 Secret Access Key                                                                                                                                            |
| `s3_public_domain` | `string`  | No       | Env: `S3_PUBLIC_DOMAIN`        | Public domain / CDN URL (e.g. `https://pub-xxxx.r2.dev`)                                                                                                                        |
...
```json
{
  "input": {
    ...
    "image_url": "https://pub-ea10d712b4c34b959b2c604b1cd8339c.r2.dev/Frases/imagen.png",
    "wav_url": "https://pub-ea10d712b4c34b959b2c604b1cd8339c.r2.dev/Frases/audio_corto.wav",
    "width": 288,
    "height": 512,
    "fps": 25,
    "steps": 6,
...
```json
{
  "input": {
    "input_type": "video",
    "person_count": "multi",
    "prompt": "Two people singing a duet.",
    "video_url": "https://example.com/input_video.mp4",
    "wav_url": "https://example.com/audio1.wav",
    "wav_url_2": "https://example.com/audio2.wav",
    "width": 512,
...
When the video generation completes successfully, the handler automatically selects the final assembled video with audio (`*-audio.mp4`), uploads it to your S3 / Cloudflare R2 bucket, and returns the direct public URL.

| Parameter   | Type     | Description                                                     |
| ----------- | -------- | --------------------------------------------------------------- |
| `video_url` | `string` | Public URL to stream and download the final generated MP4 video |

**Success Response Example:**

```json
{
  "video_url": "https://pub-ea10d712b4c34b959b2c604b1cd8339c.r2.dev/outputs/infinitetalk_task_a1b2c3d4.mp4"
}
```

#### Error

If the job fails, it returns a JSON object containing an error description.
...
```json
{
  "error": "Error al subir el video a S3: ..."
}
```
